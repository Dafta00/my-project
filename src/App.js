...
function Nav({ page, setPage, authed, user, onSignOut }) {
  ...
  return (
    <nav className="nav">
      <button className="nav-logo" onClick={() => setPage("landing")}>Career<span>AI</span></button>
      {!authed ? (
...
function Landing({ setPage }) {
  ...
  const stats = [["50K+", "Students guided"], ["1,200+", "Career paths mapped"], ["94%", "Satisfaction rate"], ["4.9★", "Average rating"]];
  return (
    <div>
      <div className="hero">
        <div className="hero-grid" />
        <div className="hero-glow" />
        <div className="hero-eyebrow"><div className="hero-eyebrow-dot" />AI-powered career intelligence</div>
        <h1 className="hero-title">Discover the career<br /><span className="grad">you were built for</span></h1>
        <p className="hero-sub">CareerAI analyses your skills, interests, and goals to surface personalised career paths, skill gaps, and learning roadmaps.</p>
        <div className="hero-actions">
          <button className="btn btn-primary btn-lg" onClick={() => setPage("auth")}>Start for free →</button>
          <button className="btn btn-ghost btn-lg" onClick={() => setPage("dashboard")}>View demo</button>
        </div>
      </div>
      <div className="stats-row">
        {stats.map(([num, label]) => (<div className="stat" key={label}><div className="stat-num">{num}</div><div className="stat-label">{label}</div></div>))}
      </div>
    ...
    try {
      if (tab === "signup") {
        ...
        setDoc(doc(db, "users", cred.user.uid), {
          ...
          createdAt: serverTimestamp(), skills: [], interests: [], goal: "", eduLevel: "undergraduate",
        });
        setUser(cred.user);
        setAuthed(true);
        setPage("profile");
    ...
    <div className="auth-page">
      ...
      <div className="auth-card">
        ...
        <div className="auth-tabs">
          <button className={"auth-tab" + (tab === "signin" ? " active" : "")} onClick={() => { setTab("signin"); setError(""); setResetSent(false); }}>Sign in</button>
          <button className={"auth-tab" + (tab === "signup" ? " active" : "")} onClick={() => { setTab("signup"); setError(""); setResetSent(false); }}>Sign up</button>
        </div>

        {tab === "signup" && (
          <div className="form-row" style={{ marginBottom: 16 }}>
            <div><label className="label">First name</label><input className="input" placeholder="John" value={first} onChange={(e) => setFirst(e.target.value)} /></div>
            <div><label className="label">Last name</label><input className="input" placeholder="Doe" value={last} onChange={(e) => setLast(e.target.value)} /></div>
          </div>
        )}
        ...
        {resetSent && (
          <div style={{ background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.25)", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "#34d399", marginBottom: 14 }}>
        ...
        {tab === "signin" && (
          <div className="forgot-link">
            <button className="auth-link" onClick={handleForgotPassword} disabled={resetLoading}>
        ...
        {resetSent && (
          <button className="btn btn-ghost btn-full" onClick={() => setResetSent(false)} style={{ marginBottom: 16 }}>
            ← Back to sign in
          </button>
        )}
...
function Profile({ setPage, user }) {
  ...
  useEffect(() => {
    ...
    getDoc(doc(db, "users", user.uid)).then((snap) => {
      if (snap.exists()) {
        ...
        setSkills(d.skills || []); setInterests(d.interests || []);
        setEduLevel(d.eduLevel || "undergraduate"); setGoal(d.goal || "");
        setChallenge(d.challenge || ""); setWorkStyle(d.workStyle || "Remote first");
  ...
  async function saveProfile() {
    if (!user) return;
    setSaving(true);
    await setDoc(doc(db, "users", user.uid), { firstName, lastName, skills, interests, eduLevel, goal, challenge, workStyle, fieldOfStudy, updatedAt: serverTimestamp() }, { merge: true });
    setSaving(false); setSaved(true); setTimeout(() => setSaved(false), 2000);
  }

  const progress = (step / (steps.length - 1)) * 100;
  const subtitles = ["Tell us a bit about yourself.", "What skills do you currently have?", "What areas excite you most?", "Paint a picture of where you are headed.", "Let AI read your CV to boost accuracy."];
  ...
  return (
    <div className="profile-page">
      <div className="profile-top">
        ...
        <div className="progress-track"><div className="progress-fill" style={{ width: progress + "%", background: "linear-gradient(90deg, var(--accent), var(--accent2))" }} /></div>
      </div>
      <div className="profile-layout">
        <div className="profile-sidebar">
          ...
          <div className="profile-steps">
            {steps.map((s, i) => (
              <button key={s} className={"step-item" + (i === step ? " active" : i < step ? " done" : "")} onClick={() => i <= step && setStep(i)}>
        ...
        <div>
          ...
          <div className="card" style={{ padding: 28 }}>
            {step === 0 && (
              <div>
                <div className="form-row" style={{ marginBottom: 16 }}>
                  <div><label className="label">First name</label><input className="input" placeholder="John" value={firstName} onChange={(e) => setFirstName(e.target.value)} /></div>
                  <div><label className="label">Last name</label><input className="input" placeholder="Doe" value={lastName} onChange={(e) => setLastName(e.target.value)} /></div>
                </div>
                <div className="form-group"><label className="label">Email</label><input className="input" type="email" value={user?.email || ""} disabled style={{ opacity: 0.6 }} /></div>
                <div className="form-group">
                ...
                <div className="form-group"><label className="label">Field of study / current role</label><input className="input" placeholder="e.g. Computer Science, Software Engineer..." value={fieldOfStudy} onChange={(e) => setFieldOfStudy(e.target.value)} /></div>
              </div>
            )}
            {step === 1 && (
              <div>
                <div className="skill-input-row">
                  <input className="input" placeholder="Type a skill and press Add..." value={skillInput} onChange={(e) => setSkillInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addSkill()} />
                  <button className="btn btn-outline" onClick={addSkill}>Add</button>
                </div>
                <div style={{ fontSize: 13, color: "var(--muted2)", marginBottom: 10 }}>Add your current skills — be honest, this helps us match you accurately.</div>
                <div className="skills-list">{skills.map((s) => (<div className="skill-tag" key={s}>{s}<button className="skill-remove" onClick={() => removeSkill(s)}>×</button></div>))}</div>
              </div>
            )}
            {step === 2 && (
              <div>
                ...
                <div className="interest-grid">
                  {interestOptions.map(({ emoji, label }) => (
                    <div key={label} className={"interest-card" + (interests.includes(label) ? " selected" : "")} onClick={() => toggleInterest(label)}>
                      <div className="interest-emoji">{emoji}</div><div>{label}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {step === 3 && (
              <div>
                <div className="form-group"><label className="label">Where do you see yourself in 5 years?</label><textarea className="input textarea" placeholder="e.g. I want to be a senior data scientist..." value={goal} onChange={(e) => setGoal(e.target.value)} /></div>
                <div className="form-group"><label className="label">What is your biggest career challenge right now?</label><textarea className="input textarea" placeholder="e.g. I do not know which direction to take after graduation..." value={challenge} onChange={(e) => setChallenge(e.target.value)} /></div>
                <div className="form-group">
                  ...
                  <select className="input select" value={workStyle} onChange={(e) => setWorkStyle(e.target.value)}>
                    <option>Remote first</option><option>Hybrid</option><option>On-site</option><option>No preference</option>
                  </select>
                </div>
              </div>
            )}
            {step === 4 && (
              <div>
                ...
                <div style={{ fontSize: 13, color: "var(--muted2)", textAlign: "center" }}>You can skip this step and upload later from your dashboard.</div>
              </div>
            )}
          </div>
          <div className="profile-nav">
            <button className="btn btn-ghost" onClick={() => (step > 0 ? setStep(step - 1) : setPage("landing"))}>← {step === 0 ? "Back" : "Previous"}</button>
            <div style={{ display: "flex", gap: 10 }}>
              <button className="btn btn-outline" onClick={saveProfile} disabled={saving}>{saving ? "Saving..." : saved ? "✓ Saved!" : "Save progress"}</button>
              <button className="btn btn-primary" onClick={() => { saveProfile(); step < steps.length - 1 ? setStep(step + 1) : setPage("dashboard"); }}>
...
function Dashboard({ setPage, user }) {
  ...
  useEffect(() => {
    if (!user) return;
    getDoc(doc(db, "users", user.uid)).then((snap) => { if (snap.exists() && snap.data().firstName) setUserName(snap.data().firstName); });
  }, [user]);
  ...
  function sendMessage() {
    ...
    setChatInput("");
    setTimeout(() => { setMessages((prev) => [...prev, { role: "ai", text: "That is a great question! Let me analyse your profile and get back to you with personalised insights..." }]); }, 800);
  }
  ...
  const mainLinks = [{ icon: "🏠", label: "Overview", active: true }, { icon: "🎯", label: "Career matches" }, { icon: "🗺️", label: "Roadmaps" }, { icon: "📈", label: "Skill tracker" }, { icon: "📚", label: "Resources" }];
  const accountLinks = [{ icon: "🔔", label: "Notifications" }, { icon: "👤", label: "Profile", action: () => setPage("profile") }, { icon: "⚙️", label: "Settings" }];
    ...
    <div className="dashboard">
      <aside className="app-sidebar">
        <div className="sidebar-section">
          ...
          {mainLinks.map((l) => (<button key={l.label} className={"sidebar-link" + (l.active ? " active" : "")} onClick={l.action}><span className="sidebar-icon">{l.icon}</span>{l.label}</button>))}
        </div>
        <div className="sidebar-section">
          <div className="sidebar-section-label">Account</div>
          {accountLinks.map((l) => (<button key={l.label} className="sidebar-link" onClick={l.action}><span className="sidebar-icon">{l.icon}</span>{l.label}</button>))}
        </div>
        <div className="sidebar-footer">
          <div className="sidebar-promo">
            <div style={{ fontSize: 12, fontWeight: 600, color: "#7eb5ff", marginBottom: 6 }}>Profile 60% complete</div>
            <div className="progress-track" style={{ marginBottom: 8 }}><div className="progress-fill" style={{ width: "60%", background: "var(--accent)" }} /></div>
      ...
      <main className="dashboard-main">
        ...
        <div className="dash-grid">
          <div>
            <div className="section-hd"><div className="section-hd-title">Top career matches</div><button className="btn btn-ghost btn-sm">View all</button></div>
            {careers.map((c) => (
              <div key={c.name} className="career-card">
                ...
                <div className="career-info">
                  ...
                  <div className="career-tags">{c.tags.map((t) => (<span key={t.text} className={"tag " + t.cls}>{t.text}</span>))}</div>
                </div>
                <div><div className="career-match">{c.match}%</div><div className="career-match-label">match</div></div>
              </div>
            ))}
          </div>
          <div className="chat-panel">
            <div className="chat-header">
              ...
              <div><div className="chat-name">CareerAI Advisor</div><div className="chat-status">● Online</div></div>
            </div>
            <div className="chat-messages">
              {messages.map((m, i) => (<div key={i} className={"chat-msg " + m.role}><div className="chat-bubble">{m.text}</div></div>))}
              <div ref={messagesEndRef} />
            </div>
            <div className="chat-input-row">
              <input className="chat-input" placeholder="Ask me anything..." value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()} />
              <button className="chat-send" onClick={sendMessage}>→</button>
            </div>
          </div>
        </div>
        <div className="skill-gap-card">
          <div className="section-hd"><div className="section-hd-title">Skill gap — Data Scientist</div><span className="tag tag-blue">Target role</span></div>
          {skillGaps.map((s) => (
            <div key={s.name} className="skill-row">
              <div className="skill-name">{s.name}</div>
              <div className="skill-bar"><div className="skill-fill" style={{ width: s.current + "%", background: s.color }} /></div>
              <div className="skill-pct">{s.current}%</div>
            </div>
          ))}
          <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 10 }}>Bars show your current skill level vs target role requirements.</div>
        </div>
      </main>
    </div>
  );
...
export default function App() {
  ...
  async function handleSignOut() {
    await signOut(auth);
    setAuthed(false); setUser(null); setPage("landing");
  }
